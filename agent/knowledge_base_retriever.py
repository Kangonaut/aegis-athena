...
# reranking
RERANKER_MODEL: str = "BAAI/bge-reranker-base"  # when using `SentenceTransformerRerank`
RERANKER_LLM_MODEL: str = "gpt-3.5-turbo-0125"  # when using `LLMRerank`
RERANKER_LLM_TEMPERATURE: float = 0.1
RERANKED_TOP_N: int = 3
RERANKER_CHOICE_BATCH_SIZE: int = 5
...
def build_default_query_pipeline() -> QueryPipeline:
    # weaviate
    ...
    reranker_llm = OpenAI(
        ...
        temperature=RERANKER_LLM_TEMPERATURE
    )
    reranker = LLMRerank(
        llm=reranker_llm,
        top_n=RERANKED_TOP_N,
...
class QueryPipelineKnowledgeBaseRetriever(BaseKnowledgeBaseRetriever):
    ...
    def from_defaults(
        ...
        return cls(
            query_pipeline=query_pipeline or build_default_query_pipeline(),
